...
async function startServer() {
  ...
  const emailUser = process.env.EMAIL_USER || 'gdelgadovisual@gmail.com';
  const emailPass = process.env.EMAIL_PASS || 'kigh cnkd wkxi epgs';
  // Priorizamos el correo del usuario como receptor principal si no hay uno configurado
  const receiverEmail = process.env.RECEIVER_EMAIL || 'gdelgadovisual@gmail.com';
