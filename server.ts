...
async function startServer() {
  ...
  // Configuración de Nodemailer - Solución definitiva para Render (IPv4 Fijo)
  ...
  // Intentamos resolver la IP dinámicamente pero forzando IPv4
  // Si falla, usaremos una IP de respaldo conocida de Google
  ...
  try {
    ...
    if (addresses && addresses.length > 0) {
      ...
      console.log(`[Server] DNS Resolve4 exitoso: ${smtpHost}`);
    }
  } catch (e) {
    console.warn('[Server] No se pudo resolver smtp.gmail.com vía IPv4, usando IP de respaldo.');
    smtpHost = '74.125.142.108'; // IP de respaldo de Google SMTP
  }
  ...
  const transporter = nodemailer.createTransport({
    ...
    tls: {
      servername: 'smtp.gmail.com', // CRUCIAL: Para que el certificado SSL coincida
