...
import { 
  ...
  Upload, MapPin, Truck, Building2, FileText, Send, Edit3, LogIn, LogOut, AlertTriangle, Loader2, Download, ExternalLink
} from 'lucide-react';
...
interface FormData {
  ...
  correo: string;
  cuotas: number;
  metodoPago: 'contado' | 'cuotas' | null;
  archivo: File | null;
...
export default function App() {
  ...
  const isFormValid = formData.nombre && formData.cedula && formData.celular && formData.correo && 
    formData.metodoPago && formData.archivo &&
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-[#2d3436] font-sans selection:bg-red-100 pb-32">
      <AnimatePresence mode="wait">
        {/* Botón Flotante para Abrir en Nueva Pestaña (Evita problemas de cookies/iframe) */}
        <div className="fixed bottom-6 right-6 z-[100] md:hidden">
          <button 
            onClick={() => window.open(window.location.href, '_blank')}
            className="bg-black text-white p-4 rounded-full shadow-2xl flex items-center gap-2 hover:scale-105 transition-all active:scale-95"
            title="Abrir en pestaña nueva para evitar errores"
          >
        ...
        <div className="fixed top-6 right-6 z-[100] hidden md:block">
          <button 
            onClick={() => window.open(window.location.href, '_blank')}
            className="bg-white/80 backdrop-blur-md border border-gray-200 text-gray-700 py-2 px-4 rounded-xl shadow-lg flex items-center gap-2 hover:bg-white transition-all active:scale-95 text-sm font-bold"
          >
          ...
          >
            <div className="max-w-2xl">
              ...
              <p className="text-xl text-gray-600 mb-8 leading-relaxed max-w-md mx-auto">
                Vive la emoción de coleccionar. Adquiere tus productos oficiales con beneficios exclusivos para ti.
              </p>
              <div className="bg-red-50 border border-red-100 rounded-2xl p-4 mb-10 flex items-center justify-center gap-3 text-red-700 max-w-sm mx-auto">
                ...
                <span className="font-bold text-sm">Beneficio FonClaro Corporativo: 12.5% de descuento aplicado</span>
              </div>
              <button
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold text-2xl text-red-600">${formatPrice(product.price * (1 - DISCOUNT_RATE))}</span>
                          </div>
                        </div>
              ...
              <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
                <div className="flex flex-col">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-black text-[#1a1a1a]">${formatPrice(subtotal)}</span>
            ...
            <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-gray-50 space-y-8">
              ...
              <section className="space-y-4">
                ...
                <div className="flex gap-4 p-1 bg-gray-100 rounded-2xl">
                  ...
                  >
                    <Truck className="w-4 h-4" /> Domicilio (Bogotá $5.000 / Otras $10.000)
                  </button>
                </div>
              ...
              <section className="space-y-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <h3 className="text-lg font-bold flex items-center gap-2 text-red-600">
                    <CreditCard className="w-5 h-5" /> Método de Pago y Documentos
                  </h3>
                  
                  {/* Resumen de Valor en esta sección */}
                  <div className="bg-red-50 px-4 py-2 rounded-xl border border-red-100 flex items-center gap-3">
                    <span className="text-xs font-bold text-red-400 uppercase">Total a Pagar:</span>
                    <span className="text-xl font-black text-red-600">${formatPrice(totalPrice)}</span>
                  </div>
                </div>
                
                <div className="flex gap-4 p-1 bg-gray-100 rounded-2xl mb-6">
                  <button 
                    onClick={() => setFormData(prev => ({ ...prev, metodoPago: 'contado', cuotas: 1 }))}
                    className={`flex-1 py-4 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${formData.metodoPago === 'contado' ? 'bg-white text-red-600 shadow-md scale-[1.02]' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    Pago de Contado
                  </button>
                  <button 
                    onClick={() => setFormData(prev => ({ ...prev, metodoPago: 'cuotas' }))}
                    className={`flex-1 py-4 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${formData.metodoPago === 'cuotas' ? 'bg-white text-red-600 shadow-md scale-[1.02]' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    A Cuotas (Nómina)
                  </button>
                </div>
                ...
                {!formData.metodoPago ? (
                  <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center">
                    ...
                    <p className="text-gray-500 font-medium">Por favor, selecciona un método de pago para continuar.</p>
                  </div>
                ) : (
                  <motion.div 
                    ...
                    {formData.metodoPago === 'contado' ? (
                      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
                        <p className="text-sm text-blue-800 leading-relaxed">
                          Para finalizar su pedido, por favor realice la consignación a favor de <strong>FonClaro Corporativo</strong>:<br />
                          <span className="block mt-2 font-bold">Cuenta Corriente Davivienda No. 100-168-20-12</span>
                          <span className="block font-bold">NIT: 830.105.648-5</span>
                          <span className="block mt-2 italic text-blue-600">Una vez realizado el pago, es obligatorio adjuntar el comprobante en este formulario para procesar su solicitud.</span>
                        </p>
                      </div>
                    ) : (
                      <div className="bg-red-50 border border-red-100 rounded-2xl p-5">
                        <p className="text-sm text-red-800 leading-relaxed mb-3">
                          Para esta modalidad, debe descargar el Formato de Convenios, diligenciarlo y firmarlo para luego adjuntarlo a este formulario.
                        </p>
                        <a 
                          href="https://docs.google.com/spreadsheets/d/11MStHgAsHN6W40JuwyyfZW3au7U4-jpw/export?format=xlsx" 
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 text-red-600 font-bold hover:underline"
                        >
                          <Download className="w-4 h-4" />
                          ¡Descarga el formato de convenios haciendo clic aquí!
                        </a>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-bold text-gray-400 uppercase ml-1">
                          {formData.metodoPago === 'contado' ? 'Adjunte el soporte de pago' : 'Adjunte el convenio firmado'}
                        </label>
                        <div 
          ...
          >
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl shadow-gray-200 border border-gray-50">
              <div className="text-center mb-10">
                ...
                <p className="text-gray-500 mb-2">Verifica que toda la información sea correcta.</p>
                <div className="bg-red-50 text-red-700 p-3 rounded-xl text-sm font-bold inline-block">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <p><span className="font-bold">Método de Pago:</span> {formData.metodoPago === 'contado' ? 'Pago de Contado' : 'A Cuotas (Nómina)'}</p>
                    {formData.metodoPago === 'cuotas' && (
                      <p><span className="font-bold">Cuotas:</span> {formData.cuotas}</p>
                    )}
                    <p>
                      <span className="font-bold">
                        {formData.metodoPago === 'contado' ? 'Soporte de pago:' : 'Convenio firmado:'}
                      </span> {formData.archivo ? formData.archivo.name : 'No adjunto'}
                    </p>
                  </div>
                </div>
                  ...
                  {selectedProducts.map((product) => {
                    ...
                    return (
                      <div key={product.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                        ...
                        <span className="font-bold text-gray-700">${formatPrice(product.price * quantity)}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="border-t border-dashed border-gray-200 pt-8 mb-10">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-medium line-through text-gray-400">${formatPrice(subtotalOriginal)}</span>
                </div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-red-600 font-bold">Descuento FonClaro Corporativo (12.5%)</span>
                  <span className="text-red-600 font-bold">-${formatPrice(subtotalOriginal * DISCOUNT_RATE)}</span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const isFormValid = 
                      ...
                      (formData.entregaTipo === 'sede' ? formData.sede !== '' : (
                    ...
                    if (!isFormValid) {
                      setSubmitError("Por favor, completa todos los campos y adjunta el archivo requerido.");
                      return;
                    }
                    ...
                    try {
                      ...
                      if (contentType && contentType.includes("application/json")) {
                        ...
                        console.error("Respuesta no JSON recibida del servidor.");
                        
                        // Detectar si es el interceptor de seguridad de AI Studio
                        if (text.includes("Cookie check") || text.includes("Action required") || text.includes("Authenticate in new window")) {
                          throw new Error("⚠️ RESTRICCIÓN DE SEGURIDAD: El navegador está bloqueando el envío desde el visor de vista previa. \n\nSOLUCIÓN: Haz clic en el botón de la esquina superior derecha (el cuadro con una flecha) para abrir la aplicación en una PESTAÑA NUEVA. Desde allí podrás enviar el pedido sin problemas.");
                        }
                        
                        throw new Error(`Error de comunicación con el servidor. Por favor, intenta abrir la app en una pestaña nueva.`);
                      }

                      if (!response.ok) {
                        const errorMsg = result.details || result.error || 'Error al enviar el pedido';
                      ...
                      setCart({});
                      setFormData({
                        nombre: '', cedula: '', celular: '', correo: '', cuotas: 1, metodoPago: null, archivo: null,
                  ...
                  className="py-4 px-6 bg-red-600 text-white rounded-2xl font-bold hover:bg-red-700 transition-all active:scale-95 shadow-lg shadow-red-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSending ? (
                ...
                {submitError && (
                  <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-2xl">
                    ...
                    {(submitError.includes("RESTRICCIÓN DE SEGURIDAD") || submitError.includes("comunicación con el servidor")) && (
                      <button 
                        onClick={() => window.open(window.location.href, '_blank')}
                        className="w-full py-3 bg-red-600 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-red-700 transition-all"
                      >
                        <ExternalLink className="w-4 h-4" />
                        Intentar en Pestaña Nueva
                      </button>
                    )}
                  </div>
                )}
        ...
        {screen === 'exito' && (
          ...
          >
            <div className="bg-white p-12 rounded-[3rem] shadow-2xl max-w-lg text-center space-y-8 border border-gray-50">
              ...
              <button
                onClick={() => setScreen('inicio')}
                className="w-full py-4 bg-red-600 text-white rounded-2xl font-bold text-lg hover:bg-red-700 transition-all active:scale-95 shadow-lg shadow-red-200"
              >
                Volver al Inicio
              </button>
            </div>
      ...
      <footer className="mt-20">
        <div className="w-full bg-red-600 py-16 px-6">
          <div className="max-w-6xl mx-auto flex flex-col items-center gap-8 text-center">
            ...
            {/* Frase */}
            <p className="text-white text-2xl font-medium italic">"La prioridad eres tú"</p>
            
            {/* Texto Maranba */}
            <p className="text-white/90 font-bold text-lg uppercase tracking-wider">
              Maranba - Distribuidor Autorizado Panini
            </p>

            {/* Estado del Servidor (Para depuración en Render) */}
            <div className="flex items-center gap-2 mt-4 px-3 py-1 bg-black/20 rounded-full text-[10px] font-bold uppercase tracking-widest text-white/60">
              <div className={`w-2 h-2 rounded-full ${
                serverStatus === 'online' ? 'bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.6)]' : 
                serverStatus === 'offline' ? 'bg-red-400 shadow-[0_0_8px_rgba(248,113,113,0.6)]' : 
                'bg-yellow-400 animate-pulse'
              }`} />
              Servidor: {
                serverStatus === 'online' ? 'Conectado' : 
                serverStatus === 'offline' ? 'Desconectado (Verificar Render Web Service)' : 
                'Verificando...'
              }
            </div>
          </div>
