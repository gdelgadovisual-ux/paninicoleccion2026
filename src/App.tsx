...
import { 
  ...
  Upload, MapPin, Truck, Building2, FileText, Send, Edit3, LogIn, LogOut, AlertTriangle, Loader2, Download
} from 'lucide-react';
...
export default function App() {
  ...
  const isFormValid = formData.nombre && formData.cedula && formData.celular && formData.correo && formData.archivo &&
              ...
              <section className="space-y-4">
                ...
                <div className="flex gap-4 p-1 bg-gray-100 rounded-2xl mb-4">
                  <button 
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${formData.metodoPago === 'contado' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500'}`}
                  >
                    Pago de Contado
                  </button>
                  <button 
                    onClick={() => setFormData(prev => ({ ...prev, metodoPago: 'cuotas' }))}
                    className={`flex-1 py-3 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${formData.metodoPago === 'cuotas' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500'}`}
                  >
                    A Cuotas (Nómina)
                  </button>
                </div>
                    ...
                    try {
                      ...
                      if (contentType && contentType.includes("application/json")) {
                        ...
                        if (text.includes("Cookie check") || text.includes("Action required")) {
                          throw new Error("Sesión expirada o bloqueada por el navegador. Por favor, abre la aplicación en una PESTAÑA NUEVA usando el botón de la esquina superior derecha para poder enviar el pedido.");
                        }
                      ...
                      setCart({});
                      setFormData({
                        nombre: '', cedula: '', celular: '', correo: '', cuotas: 1, metodoPago: 'cuotas', archivo: null,
